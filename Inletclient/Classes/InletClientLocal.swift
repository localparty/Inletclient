...
public class InletclientLocal {
    ...
    let userAttributes: [UserAttribute: String]
    
    public init(restClient: RESTClient, userAttributes: [UserAttribute: String]){
    ...
    private func getDiscoveryProfileSingle (singleOfDiscoveryConsents: DiscoveryConsentsSingle) -> DiscoveryProfileSingle {
        return singleOfDiscoveryConsents.flatMap {
            ...
            return DiscoveryProfileSingle.create { (observer) in
                ...
                _ = self.restClient
                    .request(API.putDiscoveryProfile2(discoveryConsents: discoveryConsents, userAttributes: self.userAttributes))
                    .subscribe(
                        onSuccess: {
                            (discoveryProfile) in
    ...
    private func getMailboxSingle (singleOfDiscoveryProfile: DiscoveryProfileSingle) -> MailboxSingle {
        return singleOfDiscoveryProfile.flatMap {
            ...
            return MailboxSingle.create { (observer) in
                ...
                _ = self.restClient
                    .request(API.getMailbox2(discoveryProfile: tuple.discoveryProfile))
                    .subscribe(
                        onSuccess: {
                            (mailbox) in
