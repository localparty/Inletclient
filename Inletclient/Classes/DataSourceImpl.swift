...
public class InletController {
    ...
    public var datasource: UITableViewDataSource? = MemoDataSource(reusableCellIdentifier: "default", memo: "loading data now...")
    
    public var datasourceDelegate: DatasourceDelegate? = nil
        ...
        getBrandsFromInlet(
            ...
            orElse: { error in
                self.datasource = MemoDataSource(
                    ...
                    memo: error.localizedDescription)
                self.datasourceDelegate?.resetDataSource(with: self.datasource!)
        }
        )
    }
